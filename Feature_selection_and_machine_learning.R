# combine all featues from the individual tools before running this code
...
EsInfo <- read.csv("Sc_essentiality_info.csv", sep = ",", header = T, stringsAsFactors = F)
head(EsInfo)

# load combined features
features <- read.csv("features.csv", stringsAsFactors = F, sep = "\t")
head(colnames(features))

# make deciles
for (i in 2:(ncol(features))){
  sel <- features[,i]
  df <- as.data.frame(sel) %>%  mutate(quantile = ntile(sel, 10))
  df1 <- df[,ncol(df)]
  features[,i] <- df1
}
...
# assing 5 to NAs
for (i in 2:ncol(features)) {
  features[,i][which(is.na(features[,i]))] <- 5
}

# combine duplicated entries if there are any
DUP <- Data[duplicated(Data$Gene),]
Data <- Data[!duplicated(Data$Gene),]

Data[,1002]
df <- data.frame(Genes = names(tapply(as.numeric(DUP[,2]), DUP$Gene, median)))
for (i in 2:ncol(Data)) {
  df[,i] <- tapply(as.numeric(DUP[,i]), DUP$Gene, median)
}

Data <- rbind(Data,df)

Data <- features[complete.cases(features),]
rownames(Data) <- Data$Gene

# assign class labels
Data$label <- EsInfo$label[match(Data$Gene, EsInfo$Gene)]

# randomize Data
set.seed(69)
Data <- Data[sample(1:nrow(Data), size = nrow(Data), replace = F),]
Data$label[which(Data$label == "Non-essential")] <- "NonEssential"
